...
const ListUser = () => {
  ...
  const canEdit = userPermissions.includes("Edit");
  const canDelete = userPermissions.includes("Delete");
  const mimicUserPermission = userPermissions.includes("Mimic");
  ...
  }, [dispatch, currentPage, pageSize, sortBy, sortOrder, searchTerm, filters]);

  // Handle success/error messages
  useEffect(() => {
    if (successMessage) {
      toast.success(successMessage, {
        icon: successMessage.includes('deleted') ? "🗑️" : 
              successMessage.includes('Mimic') ? "🧑‍💼" : "✅",
        duration: 3000,
      });
      
      // Handle mimic success
      if (successMessage.includes('Mimic')) {
        setTimeout(() => {
          window.location.reload();
        }, 1500);
      }
  ...
  const handleMimic = async (id) => {
    ...
    const result = await dispatch(mimicUser(id));
    
    if (mimicUser.fulfilled.match(result)) {
      const { token, roleName, roleID, permissions } = result.payload;
  ...
  const clearColumnFilter = (columnName) => {
    dispatch(removeFilter(columnName));
  };
